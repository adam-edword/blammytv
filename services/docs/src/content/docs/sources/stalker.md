...
offer instead of, or alongside, Xtream. BlammyTV speaks it directly — you do
not need the box.
...
The MAC is the credential here. Portals authorise a specific device address
rather than a username and password, which means anyone with your MAC has your
subscription — treat it exactly as you would a password.
...
If you ever move the portal to a different host, remove the source and re-add
it rather than editing the URL, so the probe runs again.

## What you get

- **Channels and genres**, as the portal organises them
- **Adult genres are recognised** and hidden unless you have turned adult
  content on — portals flag these explicitly, so the filtering here is more
...
- **The handshake was refused.** Usually the MAC is not authorised on that
