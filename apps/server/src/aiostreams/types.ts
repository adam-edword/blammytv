 ...
 * AIOStreams manifest (or any Stremio-compatible addon). We deliberately speak
 * the standard protocol, not anything AIOStreams-specific, so the VOD
 * connection isn't locked to one provider.
 *
...
export interface AddonManifest {
  ...
  name: string;
  version?: string;
  /** Each is either a bare name ("stream") or an object with type/id filters. */
  resources: Array<
...
export interface CatalogDef {
  ...
  id: string;
  name?: string;
  /** Extra params the catalog accepts (genre, skip, search …). A catalog with a
   * `search`/`required` extra isn't browseable on its own and we skip it. */
