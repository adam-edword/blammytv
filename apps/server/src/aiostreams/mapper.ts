...
import type { MetaDetail, MetaPreview, Stream, Video } from "./types.js";

// ---------------------------------------------------------------------------
// Streams → ranked sources
//
// AIOStreams already ranks, dedups and formats results, so we render them in
// the order given (the device never re-sorts). We just translate its display
// strings into our `StreamSource` shape, mirroring the glyph style the app's
// SourceCard already expects (☁︎ source · group / 🗣 langs / ◧ bitrate · size / ★).
// ---------------------------------------------------------------------------
...
/** Lightweight browse-grid entry from a catalog preview. The detail page
 * enriches it (synopsis/cast/seasons) on open via the meta endpoint. */
...
/** Fall back to reading a resolution out of the stream's `name` ("4K ⚡"). */
